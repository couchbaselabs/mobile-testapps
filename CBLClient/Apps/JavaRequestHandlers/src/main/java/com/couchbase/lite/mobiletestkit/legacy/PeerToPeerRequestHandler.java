...
public class PeerToPeerRequestHandler implements MessageEndpointDelegate {
    ...
    public Replicator configure(Args args) throws Exception {
        ...
        if (tlsAuthType.equals("self_signed")) {
            TLSIdentity tlsIdentity = TestKitApp.getApp().getSelfSignedIdentity();
            if (tlsIdentity != null) {
        ...
        if (tlsAuthenticator) {
            TLSIdentity identity = TestKitApp.getApp().getClientCertsIdentity();
    ...
    public URLEndpointListener serverStart(Args args) throws Exception {
        ...
        if (tlsAuthType.equals("self_signed_create")) {
            TLSIdentity identity = TestKitApp.getApp().getCreateIdentity();
            config.setTlsIdentity(identity);
        }

        if (tlsAuthType.equals("self_signed")) {
            TLSIdentity identity = TestKitApp.getApp().getSelfSignedIdentity();
            config.setTlsIdentity(identity);
            Log.e(TAG, "ServerSide setting the identity");
        }
        if (tlsAuthenticator) {
            List<Certificate> certsList = TestKitApp.getApp().getAuthenticatorCertsList();
