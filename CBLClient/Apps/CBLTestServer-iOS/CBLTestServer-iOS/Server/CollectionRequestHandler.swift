...
public class CollectionRequestHandler {
    ...
    public func handleRequest(method: String, args: Args) throws -> Any? {
        ...
        case "collection_createCollection":
            ...
            let database: Database = args.get(name: "database")!
            let collection: Collection = try database.createCollection(name: collectionName, scope: scopeName)
            return collection
            
        case "collection_collectionNames":
            let scope: String = (args.get(name:"scopeName"))!
            let database: Database = (args.get(name:"database"))!
            var names = [String]()
        ...
        case "collection_getDocumentExpiration":
            ...
            let collection: Collection = (args.get(name: "collection"))!
            do {
                let date = try collection.getDocumentExpiration(id: docId)
                return date
            } catch {
                return error
            }
        ...
        return CollectionRequestHandler.VOID
    }
}
